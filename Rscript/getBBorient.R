...
if (FALSE) {
  ...
  if (FALSE) {
    outnamepre<-"mix_27A_cylcssc_fibre_BB_orient"
    midfix<-"cylcssc."
  } else {
    outnamepre<-"mix_27A_fibre_BB_orient"
  }
} else {
  outnamepre<-"pure_27A_fibre_BB_orient"
  dcddir<-"/dfs9/tw/yuanmis1/mrsec/cyl1/cssc_27A/"
...
for (dcdname in dcdlist) {
  ...
  if (length(seq(1,totframe,step))>1) {
    ...
    if (dcdname!=dcdlist[length(dcdlist)]) {
      HistMatrixTemp<-getBBangHist(1)#run getConforR for first frame 
