...
source("Rscript/getHbond_spatialcutoff_perframe.R")
...
if (FALSE) {
  ...
  outputpre<-"pure_27A_fibre_hbond_spati_dist"
}
...
for (dcdname in dcdlist) {
  ...
  if (length(seq(1,totframe,step))>1) { #parallel compute with multiple frames
    RawHbond<-foreach(frame=seq(1,totframe,step)) %dopar% (
      getHbond_spatialcutoff_perframe(frame,getHist=TRUE)
    )
  } else {
    if (dcdname!=dcdlist[length(dcdlist)]) { #only one frame but not last file
      RawHbond<-list()
      RawHbond[[1]]<-getHbond_spatialcutoff_perframe(1,getHist=TRUE)
    } else {
  ...
  if (exists("outname")) {
    ...
    for (pair in names(RawHbond[[1]])) {
      ...
      for (coortype in names(RawHbond[[1]][[pair]])) {
        HbondDistHist[[pair]][[coortype]]<-0
  ...
  for (pair in names(RawHbond[[1]])) {
    for (coortype in names(RawHbond[[1]][[pair]])) {
      for (frame in 1:length(RawHbond)) {
        HbondDistHist[[pair]][[coortype]]<-HbondDistHist[[pair]][[coortype]]+RawHbond[[frame]][[pair]][[coortype]]
        if (coortype=="ResPair") {
          HbondDistHist[["dt"]]<-rbind(HbondDistHist[["dt"]],c(HbondDistHist[["Frame"]]+(frame-1)*step,sum(RawHbond[[frame]][[pair]][[coortype]]), pair, RawHbond[[frame]][[pair]][[coortype]]))
        }
