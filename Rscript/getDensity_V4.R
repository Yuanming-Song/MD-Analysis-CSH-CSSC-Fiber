...
source("Rscript/getEdge_cpp_base")
...
for (dcdname in dcdlist) {
  ...
  mydcd<-read.dcd(file.path(dcddir,dcdname),verbose = FALSE) #read dcd file
  ...
  if (length(seq(1,totframe,step))>1) { 
    rholisttemp<-foreach(frame=seq(1,totframe,step),.combine=rbind) %dopar% (
      getrho_fibre_network_v4(frame)
    )
    rholisttemp[,1]<-rholisttemp[,1]+processedframe
  } else {
    rholisttemp<-getrho_fibre_network_v4(1)
    rholisttemp[1]<-rholisttemp[1]+processedframe
