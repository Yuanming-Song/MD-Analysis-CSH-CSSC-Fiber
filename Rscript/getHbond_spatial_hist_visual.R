...
source("Rscript/getHbond_spatialcutoff_perframe_visual.R")
...
if (TRUE) {
  ...
  outputpre<-"mix_27A_fibre_hbond_spati_dist_visual"
  uniseltextlist<-c("CSSC","CSH") #unique resname in sim
  maxCSH<-max(mypdb$atom[which(mypdb$atom$resid=="CSH"),]$resno)
  if (FALSE) {
    midfix="cylcssc."
    outputpre<-"mix_27A_cylcssc_fibre_hbond_spati_dist_visual"
  }
} else {
  ...
  outputpre<-"pure_27A_fibre_hbond_spati_dist_visual"
}
...
for (dcdname in dcdlist[7]) {
  ...
  hbond_visual<-getHbond_spatialcutoff_perframe_visual(1)
  outname<-paste0(outputdir,outputpre,"_every",step,"_",regmatches(dcdname, regexpr("\\d+", dcdname)),".dat")
  write.table(hbond_visual, outname,  row.names = FALSE, quote = FALSE)
