load /pub/yuanmis1/updateHist.c UpdateHistogram
set ini 23
set fini 25
set begframe 0
set endframe 20
...
#set binmax 300.
#set binnum 300.
#set binsize [expr ($binmax - $binmin)/$binnum]
#set thing csh
#for {set i 0} {$i < $binnum } {incr i} {
#	set hist($i) 0
#}
#for {set i 0} {$i < $binnum} {incr i} {
#	lappend rho($thing) 0.0
#}
...
for {set m 0} {$m<$round} {incr m} {
	animate delete all
	if {[expr ($m + 1)*(1+$endframe - $begframe ) ] <  $lastframe} {
...
#	set outfile [open data/${ini}to${fini}rdf[expr $begframe + ${j}].dat w]
		set outfile [open data/${ini}to${fini}phe_com_frame${j}.dat w]
